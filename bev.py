...
class bev_transform_tools:
	...
	def calculate_derotation_matrix(self,M):
		...
		angle = np.arctan2((diagonal_line[1][1]-diagonal_line[0][1]), (diagonal_line[1][0]-diagonal_line[0][0]))
		rotation = np.array([[np.cos(angle) , -np.sin(angle),0],
				     [np.sin(angle), np.cos(angle),0],
				     [0,0,1]])
		self.dero = rotation
		return rotation
	def calculate_detranslation_matrix(self,rotation_matrix,target_in_pixels):
		# There are 4 steps to calculate_detranslation_matrix: 
		...
		# The returned detranslation will translate the new origin back to its original location, which is (width/2, height)
	...
	def calculate_transform_matrix(self,tile_coords):
		...
		max_width_tile = max(np.linalg.norm(top_left_tile - top_right_tile),np.linalg.norm(bot_left_tile-bot_right_tile))
		max_height_tile = max(np.linalg.norm(top_left_tile - bot_left_tile),np.linalg.norm(bot_right_tile-top_right_tile))
		zoom_out_ratio = 3
		...
		translation = self.calculate_detranslation_matrix(rotation,target_after_transform)
	...
	def create_occupancy_grid(self,segmap):
		...
		warped_img = cv2.warpPerspective(segmap,self.__intrinsic_matrix,(self.width,self.height))
		...
		warped_img  = cv2.rectangle(warped_img, (int(warped_width/2-100/self.pixel_to_cm),int(warped_height-170/self.pixel_to_cm)), \
									(int(warped_width/2+100/self.pixel_to_cm),int(warped_height-50/self.pixel_to_cm)), front_value, -1)
		...
		occupancy_grid = cv2.resize(warped_img,(self.__occ_grid,self.__occ_grid),interpolation=cv2.INTER_NEAREST)*100
